...
#EDIT THESE TO OUR OWN REFERENCE FILES
...
#keep_list<-data.frame(Name=gsub('_.*','',list.files('/scratch/groups/biomarkers-brc-mh/Reference_data/1KG_Phase3/PLINK/super_pop_keep_files')),
#                      File=paste0('/scratch/groups/biomarkers-brc-mh/Reference_data/1KG_Phase3/PLINK/super_pop_keep_files/',list.files('/scratch/groups/biomarkers-brc-mh/Reference_data/1KG_Phase3/PLINK/super_pop_keep_files')))
...
#library(data.table)
#psam<-fread('/scratch/groups/biomarkers-brc-mh/Reference_data/1KG_Phase3/PLINK/all_phase3.psam')
#names(psam)[1]<-'sample'
#psam<-psam[,c('sample','Population','SuperPop'),with=F]
#names(psam)<-c('sample','pop','super_pop')
...
if(!is.na(opt$target_keep)){
  ...
  if(is.na(opt$target_fam)){
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --threads 1 --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
      opt$target_plink_chr<-paste0(opt$output_dir,'target_subset.chr')
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --threads 1 --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset --memory ',floor(opt$memory*0.7)))
      opt$target_plink<-paste0(opt$output_dir,'target_subset')
    }
  } else {
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --threads 1 --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
      opt$target_plink_chr<-paste0(opt$output_dir,'target_subset.chr')
      opt$target_fam<-NA
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --threads 1 --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset --memory ',floor(opt$memory*0.7)))
      opt$target_plink<-paste0(opt$output_dir,'target_subset')
      opt$target_fam<-NA
    }
  }
...
if(nrow(fam) > 100){
  if(is.na(opt$target_fam)){
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --threads 1 --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --threads 1 --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  } else {
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --threads 1 --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
        system(paste0(opt$plink,' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --threads 1 --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  }
} else {
  ...
  if(is.na(opt$target_fam)){
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --threads 1 --geno ',opt$geno,' --write-snplist --allow-extra-chr --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --threads 1 --geno ',opt$geno,' --write-snplist --allow-extra-chr --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  } else {
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --threads 1 --geno ',opt$geno,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --threads 1 --geno ',opt$geno,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  }
}
...
for(i in 1:22){
  system(paste0(opt$plink,' --bfile ', opt$ref_plink_chr,i,' --threads 1 --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --make-bed --extract ',opt$output_dir,'target_QC.snplist --allow-extra-chr --out ',opt$output_dir,'ref_intersect_chr',i,' --memory ',floor(opt$memory*0.7)))
}
...
# Merge
if(dim(flip)[1] > 0){
  system(paste0(opt$plink,' --merge-list ',opt$output_dir,'ref_mergelist.txt --extract ',opt$output_dir,'ref_allele_match.snplist --flip ',opt$output_dir,'ref_flip.snplist --threads 1 --make-bed --allow-extra-chr --out ',opt$output_dir,'ref_merge --memory ',floor(opt$memory*0.7)))
} else {
  system(paste0(opt$plink,' --merge-list ',opt$output_dir,'ref_mergelist.txt --extract ',opt$output_dir,'ref_allele_match.snplist --threads 1 --make-bed --allow-extra-chr --out ',opt$output_dir,'ref_merge --memory ',floor(opt$memory*0.7)))
}
sink(file = paste(opt$output,'.log',sep=''), append = T)
cat('Done!\n')
sink()
...
system(paste0(opt$plink,' --bfile ',opt$output_dir,'ref_merge --threads 1 --exclude ',opt$output_dir,'long_ld.exclude --indep-pairwise 1000 5 0.2 --allow-extra-chr --out ',opt$output_dir,'ref_merge --memory ',floor(opt$memory*0.7)))
...
system(paste0(opt$plink,' --bfile ',opt$output_dir,'ref_merge --threads 1 --extract ',opt$output_dir,'ref_merge.prune.in --make-bed --allow-extra-chr --out ',opt$output_dir,'ref_merge_pruned --memory ',floor(opt$memory*0.7)))

# Calculate SNP weights
system(paste0(opt$plink2,' --bfile ',opt$output_dir,'ref_merge_pruned --threads 1 --pca ',opt$n_pcs,' allele-wts  --allow-extra-chr --out ',opt$output_dir,'ref_merge_pruned --memory ',floor(opt$memory*0.7)))

# Calculate PCs in the reference
system(paste0(opt$plink2,' --bfile ',opt$output_dir,'ref_merge_pruned --threads 1 --score ',opt$output_dir,'ref_merge_pruned.eigenvec.allele header-read 2 5 --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'ref_merge_pruned_score --memory ',floor(opt$memory*0.7)))
...
system(paste0('mv ',opt$output_dir,'ref_merge_pruned.eigenvec.allele ',opt$output,'.eigenvec.allele'))
...
system(paste0('cut -f 2 ',opt$output,'.eigenvec.allele | tail -n +2 > ',opt$output_dir,'score_file.snplist'))

if(is.na(opt$target_fam)){
  if(is.na(opt$target_plink)){
    for(i in 1:22){
      system(paste0(opt$plink2, ' --bfile ',opt$target_plink_chr,i,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation --threads 1 --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles.chr',i,' --memory ',floor(opt$memory*0.9)))
    }
  } else {
    system(paste0(opt$plink2, ' --bfile ',opt$target_plink,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation --threads 1 --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles --memory ',floor(opt$memory*0.9)))
  }
} else {
  if(is.na(opt$target_plink)){
    for(i in 1:22){
      system(paste0(opt$plink2, ' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation --threads 1 --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles.chr',i,' --memory ',floor(opt$memory*0.9)))
    }
  } else {
    system(paste0(opt$plink2, ' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation --threads 1 --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles --memory ',floor(opt$memory*0.9)))
  }
}
...
if(is.na(opt$target_plink)){
  ...
  var_list<-fread(paste0(opt$output,'.eigenvec.allele'))
  nsnp_all<-0
  for(i in 1:22){
