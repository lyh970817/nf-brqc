...
if(!is.na(opt$target_keep)){
  ...
  if(is.na(opt$target_fam)){
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
      opt$target_plink_chr<-paste0(opt$output_dir,'target_subset.chr')
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset --memory ',floor(opt$memory*0.7)))
      opt$target_plink<-paste0(opt$output_dir,'target_subset')
    }
  } else {
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
      opt$target_plink_chr<-paste0(opt$output_dir,'target_subset.chr')
      opt$target_fam<-NA
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --keep ',opt$target_keep,' --allow-extra-chr --make-bed --out ',opt$output_dir,'target_subset --memory ',floor(opt$memory*0.7)))
      opt$target_plink<-paste0(opt$output_dir,'target_subset')
      opt$target_fam<-NA
    }
  }
...
if(nrow(fam) > 100){
  if(is.na(opt$target_fam)){
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  } else {
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
        system(paste0(opt$plink,' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  }
} else {
  ...
  if(is.na(opt$target_fam)){
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --geno ',opt$geno,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --geno ',opt$geno,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  } else {
    if(is.na(opt$target_plink)){
      for(i in 1:22){
        system(paste0(opt$plink,' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --geno ',opt$geno,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC.chr',i,' --memory ',floor(opt$memory*0.7)))
      }
    } else {
      system(paste0(opt$plink,' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --geno ',opt$geno,' --write-snplist --allow-extra-chr --out ',opt$output_dir,'target.QC --memory ',floor(opt$memory*0.7)))
    }
  }
}
...
for(i in 1:22){
  system(paste0(opt$plink,' --bfile ', opt$ref_plink_chr,i,' --geno ',opt$geno,' --maf ',opt$maf,' --hwe ',opt$hwe,' --make-bed --extract ',opt$output_dir,'target_QC.snplist --allow-extra-chr --out ',opt$output_dir,'ref_intersect_chr',i,' --memory ',floor(opt$memory*0.7)))
}
...
# Merge
if(dim(flip)[1] > 0){
system(paste0(opt$plink,' --merge-list ',opt$output_dir,'ref_mergelist.txt --extract ',opt$output_dir,'ref_allele_match.snplist --flip ',opt$output_dir,'ref_flip.snplist  --make-bed --allow-extra-chr --out ',opt$output_dir,'ref_merge --memory ',floor(opt$memory*0.7)))
} else {
system(paste0(opt$plink,' --merge-list ',opt$output_dir,'ref_mergelist.txt --extract ',opt$output_dir,'ref_allele_match.snplist --make-bed --allow-extra-chr --out ',opt$output_dir,'ref_merge --memory ',floor(opt$memory*0.7)))
system(paste0(opt$plink,' --merge-list ',opt$output_dir,'ref_mergelist.txt --extract ',opt$output_dir,'ref_allele_match.snplist --flip ref_merge-merge.missnp --make-bed --allow-extra-chr --out ',opt$output_dir,'ref_merge --memory ',floor(opt$memory*0.7)))
}
...
system(paste0(opt$plink,' --bfile ',opt$output_dir,'ref_merge --exclude ',opt$output_dir,'long_ld.exclude --indep-pairwise 1000 5 0.2 --allow-extra-chr --out ',opt$output_dir,'ref_merge --memory ',floor(opt$memory*0.7)))
...
system(paste0(opt$plink,' --bfile ',opt$output_dir,'ref_merge --extract ',opt$output_dir,'ref_merge.prune.in --make-bed --allow-extra-chr --out ',opt$output_dir,'ref_merge_pruned --memory ',floor(opt$memory*0.7)))

# Calculate SNP weights
system(paste0(opt$plink2,' --bfile ',opt$output_dir,'ref_merge_pruned --pca ',opt$n_pcs,' allele-wts  --allow-extra-chr --out ',opt$output_dir,'ref_merge_pruned --memory ',floor(opt$memory*0.7)))

# Calculate PCs in the reference
system(paste0(opt$plink2,' --bfile ',opt$output_dir,'ref_merge_pruned --score ',opt$output_dir,'ref_merge_pruned.eigenvec.allele header-read 2 5 --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'ref_merge_pruned_score --memory ',floor(opt$memory*0.7)))
...
if(is.na(opt$target_fam)){
  if(is.na(opt$target_plink)){
    for(i in 1:22){
      system(paste0(opt$plink2, ' --bfile ',opt$target_plink_chr,i,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation  --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles.chr',i,' --memory ',floor(opt$memory*0.9)))
    }
  } else {
    system(paste0(opt$plink2, ' --bfile ',opt$target_plink,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation  --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles --memory ',floor(opt$memory*0.9)))
  }
} else {
  if(is.na(opt$target_plink)){
    for(i in 1:22){
      system(paste0(opt$plink2, ' --bfile ',opt$target_plink_chr,i,' --fam ',opt$target_fam,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation  --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles.chr',i,' --memory ',floor(opt$memory*0.9)))
    }
  } else {
    system(paste0(opt$plink2, ' --bfile ',opt$target_plink,' --fam ',opt$target_fam,' --extract ',opt$output_dir,'score_file.snplist --score ',opt$output,'.eigenvec.allele header-read 2 5 no-mean-imputation  --score-col-nums 6-',as.numeric(opt$n_pcs)+5,' --allow-extra-chr --out ',opt$output_dir,'profiles --memory ',floor(opt$memory*0.9)))
  }
}
